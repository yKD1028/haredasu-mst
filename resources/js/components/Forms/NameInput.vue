<template>
    <div class="form" :class="formChange">
        ...
        <input type="text" name="" id="name" placeholder="晴れ出す太郎" v-model="name" v-on:focus="onFocus" v-on:blur="onBlur">
...
export default {
    ...
    watch: {
        name(name) {
            ...
            var pattern = /^[A-Za-zＡ-Ｚａ-ｚ\u4E00-\u9FA0\u3005-\u3007\u3041-\u3094\u309D\u309E\u30A1-\u30F6\u30FB-\u30FE]+$/;
            var err = true;
            if(!name) {
                this.$set(this.errors, 'name', '氏名を入力してください。');
            } else if(!name.match(pattern)) {
                this.$set(this.errors, 'name', '氏名に使用できない文字が含まれています。');
