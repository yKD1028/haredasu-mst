<template>
    <div class="form" :class="formChange">
        ...
        <input type="text" name="" id="nowPass" placeholder="パスワード" v-model="nowPass" v-on:focus="onFocus" v-on:blur="onBlur">
    ...
    methods: {
        ...
        onBlur: function(){
            ...
            if(this.nowPass.match(pattern)){
                ...
                this.$set(this.errors, 'nowPass', 'パスワードを入力してください。');
                this.colorChange('form_err');
            } else {
                this.$set(this.errors, 'nowPass', 'パスワードは8字以上、20字以内で入力してください。');
                this.colorChange('form_err');
            }
        },
    },
    watch: {
        nowPass(nowPass) {
            ...
            if(!nowPass) {
                ...
                this.$set(this.errors, 'nowPass', 'パスワードには半角英数字しか利用できません。');
            } else if(!nowPass.match(pattern2)) {
                this.$set(this.errors, 'nowPass', 'パスワードは半角英字と、数字の両方を含めてください。');
            } else {
